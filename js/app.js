...
function manualBtn(key) {
  ...
  if (!url) return "";
  return `<a class="btn ghost" href="${esc(url)}" target="_blank" rel="noopener">📖 マニュアル（スライド）</a>`;
}
function workbookBtn(key) {
  const url = (CONFIG.WORKBOOK_LINKS || {})[key];
  if (!url) return "";
  return `<a class="btn ghost" href="${esc(url)}" target="_blank" rel="noopener">📒 ワークブック</a>`;
}
...
function openLog(card) {
  const canSubmit = CONFIG.LOG_FORM_URL && CONFIG.LOG_ENTRY_CARD && CONFIG.LOG_ENTRY_TEXT;
  openModal("📓 思考ログを書く", `
    <p class="hint">書くのは1回だけ。${canSubmit
      ? "<b>提出する</b>を押すと、書いた内容が入った状態で先生のフォームが開く（送信を押すだけ）。同時にマイログにも自動で残る。"
      : "書いたら<b>提出する</b>——コピーされるので、先生指定のフォームに貼って送信。同時にマイログにも自動で残る。"}読めるのは<b>先生だけ</b>——他の生徒には見えない。</p>
    <textarea id="log-text">${esc(logTemplate(card))}</textarea>
    <div class="btnrow">
      <button class="primary" id="submit-log">📤 提出する（マイログにも残る）</button>
      <button id="copy-log">📋 コピーだけ</button>
    </div>
  `);
  ...
  document.getElementById("submit-log").addEventListener("click", e => {
    ...
    if (canSubmit) {
      const url = CONFIG.LOG_FORM_URL + (CONFIG.LOG_FORM_URL.includes("?") ? "&" : "?") +
        ...
        "&" + CONFIG.LOG_ENTRY_TEXT + "=" + encodeURIComponent(ta.value);
      e.target.textContent = "✓ マイログに保存した→フォームで送信して";
      window.open(url, "_blank");
    } else {
      copyText(ta.value, e.target);
      e.target.textContent = "✓ 保存＆コピーした→フォームに貼って送信";
    }
...
function viewHome() {
  ...
  const startNodes = [
    roadNode({ href: "#/start", no: "1", title: "種候補をつくる", sub: state.seed ? "「" + state.seed + "」" : "名詞か名詞句で仮置き（AI壁打ちあり）", done: startDone1, now: firstUndoneStart === 1, ai: true }),
    ...
    roadNode({ href: "#/start", no: "3", title: "章をえらぶ", sub: rt ? rt.ch + " " + rt.name : "問いのタイプで決まる（仮決め・あとで変えてOK）", done: startDone3, now: firstUndoneStart === 3 }),
  ].join("");
  ...
  if (rt) {
    ...
    routeSection = `
      ...
      ${(manualBtn(state.route) || workbookBtn(state.route)) ? `<div class="btnrow" style="margin-top:4px">${manualBtn(state.route)}${workbookBtn(state.route)}</div>` : ""}
    `;
  } else {
    routeSection = `
  ...
  app.innerHTML = `
    ...
    ${routeSection}
    <p class="sub" style="margin-top:14px">🌀 進む順番は目安。悩んで戻るのも、<a href="#/switch">方向を変える</a>のも、正常な前進。書いたログは<a href="#/mylog">マイログ</a>にたまる。</p>
  `;
}
...
function viewStart() {
  ...
  app.innerHTML = `
    ...
    <div class="card">
      <h3>種候補がある人：ここに書く（名詞か名詞句で）</h3>
      <input type="text" id="seed-input" placeholder="例：誰も悪くない食品ロス" value="${esc(state.seed)}">
      <p class="sub" style="margin-top:6px" id="seed-status">${state.seed ? "✓ ロードマップと現在地に反映済み。提出は「ログを書く」から。" : "書くとロードマップと現在地に載る（この端末の記録）。まだない人は、下の壁打ちへ。"}</p>
    </div>
    <div class="card">
    ...
    <div class="card accent">
      ...
      <p class="sub" style="margin-top:6px" id="q-status">${state.question ? "✓ 反映済み。タイプを選んで出発しよう。" : "書くとロードマップと現在地に載る。"}</p>
      <div class="qtype">
    ...
    <p class="sub" style="margin-top:10px">🌀 決められなくても大丈夫。仮決めして進んで、違ったら<a href="#/switch">方向を変えれば</a>いい。</p>
  `;
  document.getElementById("seed-input").addEventListener("input", e => {
    state.seed = e.target.value; saveState();
    document.getElementById("seed-status").textContent = state.seed ? "✓ ロードマップと現在地に反映した。提出は「ログを書く」から。" : "書くとロードマップと現在地に載る（この端末の記録）。まだない人は、下の壁打ちへ。";
  });
  document.getElementById("q-input").addEventListener("input", e => {
    state.question = e.target.value; saveState();
    document.getElementById("q-status").textContent = state.question ? "✓ 反映した。タイプを選んで出発しよう。" : "書くとロードマップと現在地に載る。";
  });
...
function viewRoute(rid) {
  ...
  app.innerHTML = `
    ...
    <div class="card route-head r-${rid}">
      ...
      ${(manualBtn(rid) || workbookBtn(rid)) ? `<div class="btnrow">${manualBtn(rid)}${workbookBtn(rid)}</div>` : ""}
    </div>
    ...
    <p class="sub">最初から順に全部やらなくていい。今の自分のステップだけ開こう。違うと感じたら<a href="#/switch">方向を変える</a>へ。</p>
  `;
}
...
function viewStep(rid, sid) {
  ...
  app.innerHTML = `
    ...
    <p class="sub" style="margin-top:10px">🌀 悩んだら戻っていい。前のステップをやり直すのも、<a href="#/switch">問い・章ごと変える</a>のも、全部正常な前進。</p>
  `;
...
function viewMyLog() {
  ...
  app.innerHTML = `
    ...
    <h1>📓 マイログ</h1>
    <p class="sub">書いたログの控えが自動でたまる場所（新しい順）。「提出する」を押したものは先生にも届いている。<b>この一覧はこの端末の中にだけ</b>あって、他の生徒には見えない。<a href="#/about-log">ログとは？ ›</a></p>
    ${items.length === 0 ? `
...
function viewSwitch() {
  app.innerHTML = `
    <div class="crumb"><a href="#/">ロードマップ</a> › 方向を変える</div>
    <h1>🔄 方向を変える：問い・章・種は途中で変えてよい</h1>
    <div class="card">
      <h3>最初の選択は「約束」ではなく「乗り物」</h3>
      <p>方向を変えるのは失敗じゃない。君の解像度が上がった証拠（構成研究では、見立てが変わること自体が成果になる）。</p>
      <ul>
        <li>興味が消えた・ワクワクしなくなった</li>
        <li>証拠や資料がどうしても集まらない</li>
        <li>もっと良い問い・もっと合う手段が見えてしまった</li>
      </ul>
      <p>——どれかに当てはまったら、変えどきのサイン。先生に一言伝えてから移ろう。</p>
      <div class="warn">🔒 1つだけ例外：実証研究でデータを取り始めた後の仮説の書き換えは不可（後出しジャンケン）。その場合は「不支持」として報告して、次の問いへ。</div>
    </div>
    <div class="card">
      <h3>変え方</h3>
      <ul>
        <li>問いを作り直す → 問い作成AIを何度でもやり直してOK（問いカードの「見なかった方向」が次の候補になる）</li>
